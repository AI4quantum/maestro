...
def test_wow_integration():
    ...
    agent_remote_file = os.path.join(project_root, "tests", "yamls", "agents", "simple_remote_agents.yaml")
    agent_wow_file = os.path.join(project_root, "tests", "yamls", "agents", "simple_wow_agents.yaml")
    workflow_remote_file = os.path.join(project_root, "tests", "yamls", "workflows", "simple_remote_workflow.yaml")
    workflow_wow_file = os.path.join(project_root, "tests", "yamls", "workflows", "simple_wow_workflow.yaml")

    assert os.path.exists(agent_wow_file), f"Agent file not found at {agent_wow_file}"
    assert os.path.exists(agent_remote_file), f"Agent file not found at {agent_remote_file}"
    assert os.path.exists(workflow_wow_file), f"Workflow file not found at {workflow_wow_file}"
    assert os.path.exists(workflow_remote_file), f"Workflow file not found at {workflow_remote_file}"

    # Find maestro executable
    maestro_cmd = find_maestro_executable()
    if isinstance(maestro_cmd, str):
        cmd = [maestro_cmd, "serve", agent_remote_file, workflow_remote_file, "--port", "8003", "--host", "127.0.0.1"]
    else:
        cmd = maestro_cmd + ["serve", agent_remote_file, workflow_remote_file, "--port", "8003", "--host", "127.0.0.1"]
    ...
    try:
        ...
        assert wait_for_server("http://127.0.0.1:8003"), "Server failed to start within timeout"
