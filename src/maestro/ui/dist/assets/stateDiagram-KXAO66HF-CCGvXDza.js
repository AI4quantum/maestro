import { s as G, a as W, S as N } from "./chunk-TVAH2DTR-DlynSx04.js";
import { _ as f, c as t, d as H, l as S, e as P, k as z, R as _, S as U, O as C, u as F } from "./index-CTgvlc8H.js";
import { G as O } from "./graph-BtjjCyfS.js";
import { l as J } from "./layout-l28xhgRZ.js";
...
var X = f((e) => e.append("circle").attr("class", "start-state").attr("r", t().state.sizeUnit).attr("cx", t().state.padding + t().state.sizeUnit).attr("cy", t().state.padding + t().state.sizeUnit), "drawStartState"), D = f((e) => e.append("line").style("stroke", "grey").style("stroke-dasharray", "3").attr("x1", t().state.textHeight).attr("class", "divider").attr("x2", t().state.textHeight * 2).attr("y1", 0).attr("y2", 0), "drawDivider"), Y = f((e, i) => {
...
}, "addTitleAndBox"), q = f((e) => (e.append("circle").attr("class", "end-state-outer").attr("r", t().state.sizeUnit + t().state.miniPadding).attr("cx", t().state.padding + t().state.sizeUnit + t().state.miniPadding).attr("cy", t().state.padding + t().state.sizeUnit + t().state.miniPadding), e.append("circle").attr("class", "end-state-inner").attr("r", t().state.sizeUnit).attr("cx", t().state.padding + t().state.sizeUnit + 2).attr("cy", t().state.padding + t().state.sizeUnit + 2)), "drawEndState"), Z = f((e, i) => {
...
}, "drawNote"), L = f(function(e, i) {
  const d = i.id, c = { id: d, label: i.id, width: 0, height: 0 }, n = e.append("g").attr("id", d).attr("class", "stateGroup");
...
}, "drawState"), R = 0, Q = f(function(e, i, d) {
  ...
  let a = "";
  if (t().state.arrowMarkerAbsolute && (a = C(true)), x.attr("marker-end", "url(" + a + "#" + c(N.relationType.DEPENDENCY) + "End)"), d.title !== void 0) {
  ...
  R++;
}, "drawEdge"), b, T = {}, V = f(function() {
}, "setConf"), tt = f(function(e) {
  e.append("defs").append("marker").attr("id", "dependencyEnd").attr("refX", 19).attr("refY", 7).attr("markerWidth", 20).attr("markerHeight", 28).attr("orient", "auto").append("path").attr("d", "M 19,7 L9,13 L14,7 L9,1 Z");
}, "insertMarkers"), et = f(function(e, i, d, c) {
  b = t().state;
...
}, "draw"), at = f((e) => e ? e.length * b.fontSizeFactor : 1, "getLabelWidth"), A = f((e, i, d, c, n, l, x) => {
  const a = new O({ compound: true, multigraph: true });
  let s, w = true;
  for (s = 0; s < e.length; s++) if (e[s].stmt === "relation") {
    w = false;
    break;
  }
  d ? a.setGraph({ rankdir: "LR", multigraph: true, compound: true, ranker: "tight-tree", ranksep: w ? 1 : b.edgeLengthFactor, nodeSep: w ? 1 : 50, isMultiGraph: true }) : a.setGraph({ rankdir: "TB", multigraph: true, compound: true, ranksep: w ? 1 : b.edgeLengthFactor, nodeSep: w ? 1 : 50, ranker: "tight-tree", isMultiGraph: true }), a.setDefaultEdgeLabel(function() {
    return {};
  });
  ...
  for (const r of g) {
    ...
    if (u.note) {
      const y = { descriptions: [], id: u.id + "-note", note: u.note, type: "note" }, v = L(i, y, a);
  ...
  o.forEach(function(r) {
    B++, S.debug("Setting edge", r), a.setEdge(r.id1, r.id2, { relation: r, width: at(r.title), height: b.labelHeight * z.getRows(r.title).length, labelpos: "c" }, "id" + B);
  }), J(a), S.debug("Graph after layout", a.nodes());
  const m = i.node();
  a.nodes().forEach(function(r) {
    r !== void 0 && a.node(r) !== void 0 ? (S.warn("Node " + r + ": " + JSON.stringify(a.node(r))), n.select("#" + m.id + " #" + r).attr("transform", "translate(" + (a.node(r).x - a.node(r).width / 2) + "," + (a.node(r).y + (T[r] ? T[r].y : 0) - a.node(r).height / 2) + " )"), n.select("#" + m.id + " #" + r).attr("data-x-shift", a.node(r).x - a.node(r).width / 2), l.querySelectorAll("#" + m.id + " #" + r + " .divider").forEach((h) => {
...
}, "renderDoc"), it = { setConf: V, draw: et }, ht = { parser: W, get db() {
  return new N(1);
}, renderer: it, styles: G, init: f((e) => {
